...
pub fn setup_run_context() -> (Option<std::sync::Arc<std::sync::Mutex<RunContext>>>, std::thread::JoinHandle<()>) {
    let context = RunContext { event: None, thread_command: None, last_connected_event: None };
    let mutex = std::sync::Mutex::new(context);
    let context_arc = std::sync::Arc::new(mutex);
    let thread_arc = context_arc.clone();

    let context_thread = std::thread::spawn(move || {
        loop {
                    ...
                    Ok(mut manager) => {
                        match manager.open() {
                            Ok(mut controller) => {
                                ...
                                controller_guard.last_connected_event = Some(SteamControllerEvent::Connected);
                                ...
                                while !term.load(Ordering::Relaxed) {
                                    ...
                                    match controller.state(Duration::from_secs(0)) {
                                        ...
                                        Err(err) => {
                                            println!("steamy_controller controller state error: {:?}", err);

                                            let mut guard = thread_arc.lock().unwrap();
                                            guard.thread_command = Some(ThreadCommand::Stop);
                                            guard.event = Some(SteamControllerEvent::Disconnected);
                                            guard.last_connected_event = Some(SteamControllerEvent::Disconnected);
                                            std::mem::drop(guard);

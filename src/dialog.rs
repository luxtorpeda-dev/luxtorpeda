...
pub fn show_choices(title: &str, column: &str, choices: &Vec<String>) -> io::Result<(String, bool)> {
    let (window, _ctx, mut egui_ctx, mut event_pump, controller) = start_egui_window(600, 400, &title)?;
    let (mut painter, mut egui_state) = egui_backend::with_sdl2(&window, DpiScaling::Custom(1.0));
...
pub fn run_mgmt() -> Result<(), Error> {
    ...
    'running: loop {
        ...
        egui::TopBottomPanel::top("top_panel").resizable(false).show(&egui_ctx, |ui| {
            ui.vertical_centered(|ui| {
                ui.label(std::format!("luxtorpeda-dev {0}", env!("CARGO_PKG_VERSION")));
            });
        });

        egui::TopBottomPanel::bottom("bottom_panel").resizable(false).show(&egui_ctx, |ui| {
            let layout = egui::Layout::top_down(egui::Align::Center).with_cross_justify(true);
                ui.with_layout(layout,|ui| {
